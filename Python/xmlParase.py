...
def copyLinkedModel(model, tagID):
    newModel = ET.Element("TModelLink")
    newModel.set("id", str(tagID))
        ...
        if ".csv" in csvName:
            ...
            if os.path.isfile(workPath + "\\" + cmpLibFileName):
                ...
                for ele in modelTree.iter():
                    idTag = ele.get("id")
                    if idTag:
                        if int(idTag) > int(greatestID):
                            greatestID = idTag
                lastID = int(greatestID) + 1
##                        ele.set("id", str(i))
##                        i= i + 1
                    ...
                    for linkedModel in vaultIDLib.iter("TModelLink"):
                        for hridEle in linkedModel.iter("HRID"):
                            if hridEle.text == hrid:
                                cloneModel = copyLinkedModel(linkedModel, lastID)
