...
class vaultItemRevision:
    def __init__(self, RevisionGUID, Comment, ItemGUID, RevisionID, HRID, FolderGUID):
...
def getBatchVaultItemInfofromVault(vaultClient, itemList, *folderPath):
    client = vaultClient
    queryItemList = []  #add quotation mark for every item name to prepare for query string
    itemFolderList = []
    for item in itemList:
        queryItemList.append("'" + item.modelName + "'")
        if not (item.vaultFolder in itemFolderList):
            itemFolderList.append(getSpecificFolderGUID(item.vaultFolder))
    ...
    for revision in vaultItemRevisionList:
        ...
        #if the first model in the list is withou folder setting, then assuming all models are without vault folder
        if not itemList[0].vaultFolder:
            itemRevisionList.append(itemRevision)

        elif itemRevision.FolderGUID in itemFolderList:
            itemRevisionList.append(itemRevision)
...
def createComponentEle(cmpHRID, symbolList, footprintList, paramDict):
    ...
    for symbol in symbolList:
        symbolEle = createModelChoiceEle(symbol.modelName)
    ...
    for footprint in footprintList:
        footprintEle = createModelChoiceEle(footprint.modelName)
        ...
        for field in row:
            if row[field]:
                ...
                if ("PCB Name " in field):
                    ...
                    fildPCBLib = field.replace("PCB Name", "PCB Library")
                    if row.has_key[fildPCBLib]:
                        newFootprint.libName = row[fildPCBLib]
...
def buildCmpLib(crPath, cmpLibPath, crType, vaultAddress, userName, pwd):
    ...
    if (componentSet.symbolList and componentSet.footprintList):
        ...
        writeEle2File(cmpEleTree,  OP.join(cmpLibPath, OP.basename(crPath)).replace(".csv", ".CmpLib"))
