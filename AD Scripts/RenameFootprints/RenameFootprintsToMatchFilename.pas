...
Begin
    ...
    If ((ComponentCount = 1) && (LibComp.Name <> S))  Then
    Begin
       Try
          // Prepare the PCB software agents for changes.
          PCBServer.PreProcess;

          PCBServer.SendMessageToRobots(LibComp.I_ObjectAddress, c_Broadcast, PCBM_BeginModify , c_NoEventData);
          LibComp.Name := S;   //Comment this out if you only want a report
       ...
       Finally
          ...
          ReportInfo.Add('| Footprint renamed:     ' + LibComp.Name);
          ReportInfo.Add('|');
       End;
    End;
    CurrentLib.Board.ViewManager_FullUpdate();
    CurrentLib.Board.SetState_DocumentHasChanged;
...
Begin
    ...
    ReportFilename := ('G:\PCB Library Renaming Report (' + IntToStr(Yr) + '-' + IntToStr(Mo) + '-'
