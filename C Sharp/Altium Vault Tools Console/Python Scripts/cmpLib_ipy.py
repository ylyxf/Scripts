...
def getBatchVaultItemInfofromVault(vaultClient, itemList, itemType, allVaultFolders):
    client = vaultClient
    queryItemList = []  #add quotation mark for every item name to prepare for query string
    itemFolderList = []

    #if the first model in the list is without folder setting, then assuming all models are without vault folder
    if not itemList[0].vaultFolder:
        vaultFolderDefined = 0
    else: vaultFolderDefined = 1
   
    for item in itemList:
        queryItemList.append("'" + item.modelName + "'")
        if not (item.vaultFolder in itemFolderList):
            itemFolderList.append(getSpecificFolderGUID(item.vaultFolder, allVaultFolders))
    
    if itemType == "SCHLIB":
        itemTypeGUID = "CB22DA24-E317-11DF-B822-12313F0024A2"
    elif itemType == "PCBLIB":
        itemTypeGUID = "CB09A478-E317-11DF-B822-12313F0024A2"
    else:
        itemTypeGUID = ""
...
def buildCmpLib(crPath, templatePath, crType, vaultAddress, userName, pwd):
    ...
    if (componentSet.symbolList and componentSet.footprintList):
        ...
        symbolVaultItemList = getBatchVaultItemInfofromVault(client, componentSet.symbolList, "SCHLIB", allFolders)
        footprintVaultItemList = getBatchVaultItemInfofromVault(client, componentSet.footprintList, "PCBLIB", allFolders)
